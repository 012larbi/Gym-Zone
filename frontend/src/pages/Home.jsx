        ...
        <section className="relative px-6 py-16 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-4 items-center min-h-[819px]">
          ...
          <div className="lg:col-span-6 relative h-[600px] w-full mt-8 lg:mt-0 rounded-xl overflow-hidden glass-card group">
            {content.hero_image_type === 'video' ? (
              <video src={content.hero_image} className="w-full h-full object-cover opacity-60 mix-blend-luminosity group-hover:opacity-80 transition-opacity duration-700" autoPlay muted loop playsInline />
            ) : (
              <img
                alt="Gym Hero"
            ...
            <div className="absolute inset-0 border border-[#00f0ff]/20 rounded-xl pointer-events-none" />
          </div>
        </section>
            ...
            >
              <div className="absolute inset-0 w-full h-full">
                {content.feat1_image_type === 'video' ? (
                  <video src={content.feat1_image} className="w-full h-full object-cover opacity-40" autoPlay muted loop playsInline />
                ) : (
                  <img src={content.feat1_image || 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?q=80&w=2070&auto=format&fit=crop'} alt="Coaching" className="w-full h-full object-cover opacity-40" loading="lazy" />
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-[#0e141a] via-[#0e141a]/60 to-transparent" />
              </div>
              <div className="relative z-10 mt-auto p-8">
                <div className="w-12 h-12 rounded-full bg-[#00f0ff]/20 flex items-center justify-center text-[#00f0ff] mb-4 border border-[#00f0ff]/50">
                  <span className="material-symbols-outlined icon-fill">psychology</span>
                </div>
                <h3 className="font-grotesk font-semibold text-2xl text-white mb-2">{content.feat1_title || 'Coaching Stratégique'}</h3>
                <p className="font-manrope text-base text-[#b9cacb]">{content.feat1_desc || "Des protocoles d'entraînement basés sur la data biométrique pour une progression millimétrée."}</p>
              </div>
            </div>
            ...
            >
              <div className="absolute inset-0 w-full h-full">
                {content.feat2_image_type === 'video' ? (
                  <video src={content.feat2_image} className="w-full h-full object-cover opacity-40" autoPlay muted loop playsInline />
                ) : (
                  <img src={content.feat2_image || 'https://images.unsplash.com/photo-1490645935967-10de6ba17061?q=80&w=2053&auto=format&fit=crop'} alt="Nutrition" className="w-full h-full object-cover opacity-40" loading="lazy" />
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-[#0e141a] via-[#0e141a]/60 to-transparent" />
              </div>
              <div className="relative z-10 mt-auto p-8">
                <div className="w-12 h-12 rounded-full bg-[#7701d0]/20 flex items-center justify-center text-[#efdbff] mb-4 border border-[#7701d0]/50">
                  <span className="material-symbols-outlined icon-fill">restaurant_menu</span>
                </div>
                <h3 className="font-grotesk font-semibold text-2xl text-white mb-2">{content.feat2_title || 'Nutrition Précise'}</h3>
                <p className="font-manrope text-base text-[#b9cacb]">{content.feat2_desc || "Plans alimentaires calculés pour maximiser l'hypertrophie et la récupération."}</p>
              </div>
            </div>

            <div className="glass-card rounded-xl p-8 md:col-span-12 flex flex-col md:flex-row items-center gap-4 min-h-[250px] border-b-2 border-b-[#00f0ff]/30 relative overflow-hidden group">
              <div className="flex-1 z-10">
                <div className="w-12 h-12 rounded-full bg-[#00f0ff]/20 flex items-center justify-center text-[#00f0ff] mb-4 border border-[#00f0ff]/50">
                  <span className="material-symbols-outlined icon-fill">fitness_center</span>
                </div>
                <h3 className="font-grotesk font-semibold text-2xl text-white mb-2">{content.feat3_title || 'Équipement Pro-Level'}</h3>
                <p className="font-manrope text-base text-[#b9cacb]">{content.feat3_desc || 'Accès exclusif à des machines biomécaniques de dernière génération.'}</p>
              </div>
              <div className="flex-1 w-full h-full absolute md:relative inset-0 md:inset-auto z-0 opacity-20 md:opacity-50">
                {content.home_extra_image_type === 'video' ? (
                  <video src={content.home_extra_image} className="w-full h-full object-cover" autoPlay muted loop playsInline />
                ) : (
                  <img src={content.home_extra_image || 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=2070&auto=format&fit=crop'} className="w-full h-full object-cover" alt="Extra" loading="lazy" />
                )}
              </div>
            </div>
          </div>
